...
# search = dif("D:/Kaggle/CleanCodeCup/2023/2023/images/images")
# # search = dif("D:/Kaggle/CleanCodeCup/mod")
# search.show_output = True
# search.logs = True
# search.show_progress = True
# print(search.result)
# import pickle
# with open(r"someobject.pickle", "wb") as output_file:
#     pickle.dump(search, output_file)

# with open(r"someobject.pickle", "rb") as output_file:
...
class dif:
    ...
    def __init__(self, *directory, fast_search=True, recursive=True, similarity='normal', px_size=50,
                 show_progress=True, show_output=False, move_to=None, delete=False, silent_del=False, logs=False):
        '''
        Parameters
        ----------
        directory : str, list
        ...
        self.stats = dif._generate_stats(self, start_time=time.localtime(start_time), end_time=time.localtime(end_time),
                                         time_elapsed=time_elapsed,
                                         total_searched=total_count, total_matches=match_count,
    ...
    def _run(self):
        ...
        result, exclude_from_search, total_count, match_count = _search._matches(imgs_matrices, id_by_location,
                                                                                 self.similarity, self.show_output,
                                                                                 self.show_progress, self.fast_search)
                ...
                if os.path.isdir(file):
                    ...
                    try:
                        ...
                        if e.__class__.__name__ == 'UnidentifiedImageError':
                            invalid_files[
                                str(Path(file))] = f'UnidentifiedImageError: file could not be identified as image.'
                        else:
                        ...
                        while rotations <= 3:
                            ...
                            if mse <= similarity:
                                ...
                                if show_output:
                                    ...
                                    _help._show_file_info(str(Path(id_by_location[id_A])),
...
if __name__ == '__main__':
    ...
    parser.add_argument('-Z', '--output_directory', type=str,
                        help='Output directory for the difPy result files. Default is working dir.', required=False,
                        default=None)
    parser.add_argument('-f', '--fast_search', type=str, help='Use difPys Fast Search Algorithm.', required=False,
                        choices=[True, False], default=True)
    parser.add_argument('-r', '--recursive', type=bool, help='Scan subfolders for duplicate images', required=False,
                        choices=[True, False], default=True)
    parser.add_argument('-s', '--similarity', type=_help._type_str_int, help='Similarity grade.', required=False,
                        default='normal')
    parser.add_argument('-px', '--px_size', type=int, help='Compression size of images in pixels.', required=False,
                        default=50)
    parser.add_argument('-p', '--show_progress', type=bool, help='Show the real-time progress of difPy.',
                        required=False, choices=[True, False], default=True)
    parser.add_argument('-o', '--show_output', type=bool, help='Show the compared images in real-time.', required=False,
                        choices=[True, False], default=False)
    parser.add_argument('-mv', '--move_to', type=str, help='Move the lower quelity images to a target folder.',
                        required=False, default=None)
    parser.add_argument('-d', '--delete', type=bool, help='Delete all duplicate images with lower quality.',
                        required=False, choices=[True, False], default=False)
    parser.add_argument('-sd', '--silent_del', type=bool, help='Supress the user confirmation when deleting images.',
                        required=False, choices=[True, False], default=False)
    parser.add_argument('-l', '--logs', type=bool, help='Enable log collection for invalid files.', required=False,
                        choices=[True, False], default=False)
